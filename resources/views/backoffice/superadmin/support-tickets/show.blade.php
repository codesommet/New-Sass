...
@section('content')
    ...
    <div class="page-wrapper">
        <div class="content">
            <div class="mb-3">
                <h6><a href="{{ route('sa.support-tickets.index') }}"><i class="isax isax-arrow-left me-2"></i>{{ __('Aperçu du ticket') }}</a></h6>
            </div>
                        ...
                        <div class="d-flex align-items-center">
                            ...
                            <div class="dropdown">
                                <a href="javascript:void(0);"
                                    ...
                                    <i class="isax isax-shield me-1 text-gray-9"></i> {{ $ticket->status_label }}
                                </a>
                                <div class="dropdown-menu shadow-lg w-100 dropdown-info">
                                    <ul class="mb-3">
                                        @foreach (['open' => 'Ouvert', 'in_progress' => 'En cours', 'on_hold' => 'En attente', 'resolved' => 'Résolu', 'closed' => 'Fermé'] as $statusKey => $statusLabel)
                                            <li>
                                                <form method="POST" action="{{ route('sa.support-tickets.update-status', $ticket) }}">
                                                    @csrf
                                                    @method('PATCH')
                                                    <input type="hidden" name="status" value="{{ $statusKey }}">
                                                    <button type="submit" class="dropdown-item px-2 d-flex align-items-center {{ $ticket->status === $statusKey ? 'fw-bold text-primary' : '' }}">
                                                        {{ __($statusLabel) }}
                                                    </button>
                                                </form>
                                            </li>
                    ...
                    <div class="row mb-3 mx-1">
                            ...
                            <div class="p-3 border flex-fill rounded-left border-end-0">
                                <div class="d-flex align-items-center">
                                    <span class="avatar avatar-lg rounded-circle bg-primary text-white d-flex align-items-center justify-content-center me-2">
                                        {{ strtoupper(substr($ticket->user->name ?? 'A', 0, 1)) }}
                                    </span>
                                    <div>
                        ...
                        <div class="col-lg-4 p-0 d-flex">
                            <div class="p-3 border flex-fill rounded-0 border-end-0">
                                <div class="">
                                    <h6 class="fs-14 mb-1">{{ __('Tenant') }}</h6>
                                    <p class="fs-13">{{ $ticket->tenant->name ?? '-' }}</p>
                                </div>
                            </div>
                        </div>
                        <div class="col-lg-4 p-0 d-flex">
                                        ...
                                        @foreach ($ticket->replies->reverse()->take(5) as $reply)
                                            <li class="feed-item timeline-item">
                                                <p class="mb-1"><span class="text-dark fw-semibold">{{ $reply->user->name ?? __('Admin') }}
                                                    ...
                                                    @else
                                                        <span class="badge badge-soft-primary badge-sm ms-1">{{ __('Client') }}</span>
                                                    @endif
                            ...
                            <div class="d-flex align-items-center">
                                ...
                                <div>
                                    <h6 class="fs-14 mb-1">{{ $reply->user->name ?? __('Admin') }}
                                        ...
                                        @else
                                            <span class="badge badge-soft-primary ms-1">{{ __('Client') }}</span>
                                        @endif
                                    </h6>
            ...
            <form action="{{ route('sa.support-tickets.reply', $ticket) }}" method="POST">
                @csrf
                <div class="mb-3">
                    ...
                    <textarea class="form-control @error('message') is-invalid @enderror" name="message" rows="4"
                        placeholder="{{ __('Écrivez votre réponse...') }}">{{ old('message') }}</textarea>
                    @error('message')
